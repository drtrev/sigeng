...
{
  if (conf.level != .95) warning("mixed.ci: withinsubjects.ci difference==T so conf level should be .95")
  # Safety when testing
  test <- F
  if (test)
  {
...
{
  if (conf.level != .95) warning("mixed.ci: cm.ci.mixed difference==T so conf level should be .95")
  test <- F
  if (test)
  {
  # testing
